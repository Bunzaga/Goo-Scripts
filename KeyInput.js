...
var setup = function(args, ctx, goo) {
	...
	ctx.KeyInput = {};
	ctx.KeyInput.getKey = function(keyCode){
		var key = typeof keyCode === 'number' ? keyCode : ctx.stringToCode[""+keyCode];
		return keys[key];
	}
	ctx.KeyInput.bind = function(keyCode, callback){
		var key = typeof keyCode === 'number' ? keyCode : ctx.stringToCode[""+keyCode];
		ctx.keys[key] = false;
		if(callback){
			if(typeof callback === 'function'){
				ctx.callbacks[key] = callback;
			}
		}
		return ctx.KeyInput;
	}
	ctx.KeyInput.unbind = function(keyCode){
		var key = typeof keyCode === 'number' ? keyCode : ctx.stringToCode[""+keyCode];
		delete ctx.keys[key];
		delete ctx.callbacks[key];
		return ctx.KeyInput;
	}
	...
	ctx.worldData.KeyInput = ctx.KeyInput;
	_ctx = ctx;
};
